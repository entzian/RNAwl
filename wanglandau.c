...
scale_dos(gsl_histogram *);


static void
output_dos(const gsl_histogram *,
           const char);


static short
histogram_is_flat(const gsl_histogram *);


static double
partition_function(const gsl_histogram *);


static gsl_histogram *
ini_histogram_uniform(const int,
                      const double,
                      const double);
...
initialize_wl(vrna_fold_compound_t *vc)
{
  ...
  if (wanglandau_opt.res_given) {
    ...
    /* determine histogram ranges automatically */
    if (wanglandau_opt.max_given)
      hmax = wanglandau_opt.max;
    else
      hmax = 20 * fabs(mfe);
  ...
  strcpy(out_prefix, wanglandau_opt.basename);
  strcat(out_prefix, ".res");
  strcat(out_prefix, res_string);
  strcat(out_prefix, ".");
  free(res_string);
  return;
}
...
initialize_dos_estimate(void)
{
  ...
  if (wanglandau_opt.truedosbins_given) {
    ...
    if (wanglandau_opt.verbose)
      fprintf(stderr,
              "initializing the lowest %d bins of DOS estimate g with true DOS values from subopt:\n",
              wanglandau_opt.truedosbins);

    for (i = 0; i < wanglandau_opt.truedosbins; i++)
      g->bin[i] = log(s->bin[i]); /* get corresponding value from s histogram */ for (i =
                                                                                        wanglandau_opt
                                                                                        .truedosbins;
                                                                                      i < n; i++)
...
/* ==== */
static void
output_dos(const gsl_histogram  *x,
           const char           T)
{
  int     i, fnlen;
  FILE    *dos_fp = NULL;
